...
async function load(){
    ...
    let resp= await fetch(`${sessionStorage.host}/api/Ingrediente?sk=${sk}${filtro}`,{
        method: 'GET',
        headers:{
...
async function actual(id){
    let resp= await fetch(`${sessionStorage.host}/api/Ingrediente/${id}`,{
        method: 'GET',
        headers:{
            'x-auth': sessionStorage.token
        },
...
actualizar.addEventListener("click", async function(e){
    ...
    //console.log(imp);
    let resp= await fetch(`${sessionStorage.host}/api/Ingrediente/${ingredienteactual[0]._id}`,{
        method: 'PUT',
        headers:{
            'x-auth': sessionStorage.token,
...
guardaringrediente.addEventListener("click", async function(e){
    ...
    let imp=JSON.stringify(f);
    console.log(imp);
    let resp= await fetch(`${sessionStorage.host}/api/Ingrediente`,{
        method: 'POST',
        headers:{
            'x-auth': sessionStorage.token,
            'Content-Type': 'application/json'},
